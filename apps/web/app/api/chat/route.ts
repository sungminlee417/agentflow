...
function systemPromptFor(connected: string[]): string {
  ...
  lines.push(
    ...
    "- Tools: video_ideas_list_accounts (find which account to act on), video_ideas_list (browse pending/scheduled/done/dismissed ideas), video_ideas_get (full content of one), video_ideas_create (add a new idea — provide all the upload-ready content fields), video_ideas_update (rewrite hook, swap hashtags, polish script), video_ideas_reevaluate (audit an existing idea against settled reviews + preferences — returns keep/refine/drop + suggested rewrites), video_ideas_set_status (scheduled / done / dismissed), video_ideas_delete (rarely — prefer dismissed), video_ideas_evaluate (judge a raw idea spark the user pitches you — returns add/needs_work/pass with reasoning; auto-inserts on 'add').",
    "- Common workflow: when the user asks you to edit an idea AND check that it still holds, chain video_ideas_update → video_ideas_reevaluate → (if refine) video_ideas_update with the returned refined_fields → describe the final state. On 'drop' verdict, suggest video_ideas_set_status('dismissed') and explain why. On 'keep' just confirm the change is sound.",
