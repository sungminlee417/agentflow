...
function systemPromptFor(connected: string[]): string {
  ...
  lines.push(
    ...
    "- Closed performance loop: when the user has posted a video for an idea, use video_ideas_mark_posted to link the TikTok URL — this auto-schedules a +48h review. Use video_ideas_run_review to pull stats + post-mortem on demand (e.g. user asks 'how did the Hotel California video do?'). The post-mortem includes a verdict (hit / on_track / underperformed / too_early), the engagement-rate ratio vs the creator's median, and actionable takeaways. Read past reviews via video_ideas_get before suggesting new ideas — they're ground truth about what works for this audience.",
